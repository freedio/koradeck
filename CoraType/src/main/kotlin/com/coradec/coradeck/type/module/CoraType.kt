...
object CoraType: CoraModule<CoraTypeAPI>() {
    ...
    fun <P : Any?> castTo(value: Any?, type: KType): P? = impl.castTo(value, type)
    /** Casts the specified value to the specified type, if possible, preserving `null` values. */
    fun <P: Any?> castTo(value: Any?, type: KClass<*>): P? = impl.castTo(value, type)
    ...
    fun Any?.toBoolean(): Boolean = castTo(this, Boolean::class) ?: false
}
