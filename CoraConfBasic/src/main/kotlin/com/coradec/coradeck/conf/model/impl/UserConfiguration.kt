...
class UserConfiguration: Configuration {
    private val userconfig = ConcurrentHashMap(
        try {
            Configurations.loadGlobalConfiguration(Paths.get(USER_HOME).resolve(".coradec").resolve("user-config"))
    ...
    override fun <P : Any> get(type: KClass<P>, name: String): P? = CoraType.castTo(userconfig[name], type)
    override fun <P : Any> get(type: KType, name: String): P? = CoraType.castTo(userconfig[name], type)
}
