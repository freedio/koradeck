...
import com.coradec.coradeck.gui.ctrl.impl.ApplicationLayout.ApplicationSectionIndex.CONTENT_PLANE
import com.coradec.coradeck.gui.ctrl.impl.ApplicationLayout.ApplicationSectionIndex.CONTROL_PLANE
...
class ManualModelUT {
    @Test fun testManualModelConstruction() {
        // given:
        ...
        frame[CONTENT_PLANE].add(labelName, label.memberView)
        frame[CONTROL_PLANE].add(buttonName, button.memberView)
        Thread.sleep(20)
        frame.standby()
        // then:
        assertThat(gui.members).containsValue(frame)
        assertThat(frame.sections).containsExactlyInAnyOrder(CONTROL_PLANE, CONTENT_PLANE)
        assertThat(frame[CONTENT_PLANE].contains(labelName).value).isTrue()
        assertThat(frame[CONTROL_PLANE].contains(buttonName).value).isTrue()
        ...
        assertThat(button.visible).isFalse()
        assertThat(app.lookup("Main").value).isEqualTo(frame.memberView)
    }

    @Test fun testManualModelIllumination() {
        // given:
        val gui = GUIModel("Manual")
        val labelName = "Label"
        val buttonName = "Button"
        ...
        Thread.sleep(100)
        // then:
        assertThat(frame.peer.isVisible).isTrue()
    }
